...
description: "Every Socket.IO event in the /AVLTree namespace: what to emit, what you receive back, payload shapes, and real client code examples."
---
...
Delete a specific obstacle node from the AVL tree. The server locates the node by its `(x, y)` coordinates, removes it, rebalances the tree, and returns the updated tree state. On failure the response has `ok: false` and an `error` message.
...
socket.on('avl_tree_balanced', (response) => {
  if (response.ok) {
    console.log('Obstacle removed. Updated tree:', response.data);
  } else {
