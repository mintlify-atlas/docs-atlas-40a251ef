...
// Server confirms the connection is ready
socket.on('connected', (data) => {
  console.log(data);
  // {
  //   ok: true,
  //   received: null,
  //   message: 'The Socket is Connected, ready to use!',
  //   data: null,
  //   error: null
  // }
});
...
<Note>
  The `SocketManager` constructor passes `cors_allowed_origins` directly to Flask-SocketIO's `SocketIO` base class. The default configuration also enables `supports_credentials=True` through Flask-SocketIO's main constructor defaults, which is appropriate when the frontend and backend run on separate origins.
</Note>
